...
<head>
  ...
  <title>{{ config('app.name', 'ServerHop') }} - {{ __('auth.sign_in_title') }}</title>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .form-input:focus {
      outline: none; border-color: var(--blue-strong);
      box-shadow: 0 0 0 4px rgba(153, 204, 238, 0.25);
    }
    ...
    .form-hint a {
      color: var(--blue-strong); text-decoration: none; font-weight: 700;
    ...
    .submit-btn:hover {
      ...
      box-shadow: 0 18px 34px rgba(115, 176, 214, 0.42);
    }
    ...
    @media (max-width: 520px) {
      ...
      .login-shell { width: 100%; min-height: 100svh; border-radius: 0; border: none; box-shadow: none; padding: 40px 24px 32px; }
  ...
  <div>
    <main class="login-shell" role="main">
      ...
      <div class="divider-row">
        <span class="divider-text">{{ __('auth.or_sign_in_with') }}</span>
      </div>
      ...
      <p class="login-foot">
        {{ __('auth.dont_have_account') }} <a href="{{ route('register') }}" wire:navigate>{{ __('auth.sign_up') }}</a>
      </p>
      @endif
    </main>
  </div>
